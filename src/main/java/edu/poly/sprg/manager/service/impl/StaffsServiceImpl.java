...
public class StaffsServiceImpl implements StaffsService {
	...
	public List<Object[]> getPoint() {
		return staffsRepository.getPoint();
	}
	...
	public List<Object[]> getStaff() {
		return staffsRepository.getStaff();
	}

	@Override
	public List<String> seachStaff(String input) {
		List<Staffs> staffs = staffsRepository.seachStaff(input);
		List<String> strings = new ArrayList<>();

		for (Staffs item : staffs) {
			if(item.getName().equals(input)) {
				strings.add(item.getName() + " - depart : " + item.getDeparts().getName());
			}
		}
